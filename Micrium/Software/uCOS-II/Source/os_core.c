...
edf_tcb_ext idle_tcb_edf_ext = { 9999,9999,9999,10000,9999, 0 };
...
static INT16U get_next_task_id(void);
...
void  OSInit (void)
{
	...
	ready_task_heap = createHeap(10);
	wait_task_heap = createHeap(10);
    ...
    if (OSRunning == OS_TRUE) {
        ...
        if (OSIntNesting == 0u) {                          /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0u) {                     /* ... and not locked.                      */
				...
				if (((edf_tcb_ext*)OSTCBCur->OSTCBExtPtr)->is_task_complete==0) {
					INT8U next_task_prio = OS_EDFSched();
					INT16U next_task_id = get_next_task_id();
					if (next_task_prio != OSPrioHighRdy) {
						INT16U preempted_task_id = OSTCBCur->OSTCBId;
						OSPrioHighRdy = next_task_prio;
						OSTCBHighRdy = OSTCBPrioTbl[OSPrioHighRdy];
						((edf_tcb_ext*)OSTCBHighRdy->OSTCBExtPtr)->is_task_complete = 0;
						APP_TRACE("\n%d\tpreempt\t\t%d\t%d", OSTimeGet(), (int)preempted_task_id, (int)(next_task_id));
					}
				}
...
void  OSTimeTick (void)
{
	...
	if (OSRunning == OS_TRUE) {
		...
		while (!isHeapEmpty(wait_task_heap)&&wait_task_heap->data->value == current_time) {
...
void  OS_Sched (void)
{
    ...
    if (OSIntNesting == 0u) {                          /* Schedule only if all ISRs done and ...       */
        if (OSLockNesting == 0u) {                     /* ... scheduler is not locked                  */
			...
			INT16U complete_task_id = OSTCBCur->OSTCBId;
			INT8U next_task_prio = OS_EDFSched();
			OSPrioHighRdy = next_task_prio;
			OSTCBHighRdy = OSTCBPrioTbl[OSPrioHighRdy];
			INT16U next_task_id = get_next_task_id();
			((edf_tcb_ext*)OSTCBHighRdy->OSTCBExtPtr)->is_task_complete = 0;
			APP_TRACE("\n%d\tcompleted\t%d\t%d", OSTimeGet(), complete_task_id, next_task_id);
...
static INT16U get_next_task_id(void) {
	if (isHeapEmpty(ready_task_heap)) {
		return OS_TASK_IDLE_ID;
	}
	else {
		return ready_task_heap->data->id;
	}
}
